...
/*   Updated: 2018/03/12 19:13:33 by ldedier          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	ft_printf("file \"%s\"could not be loaded", str);
	exit(1);
}

SDL_Texture* ft_load_texture(t_env *e, char *str)
{
	SDL_Texture* texture = IMG_LoadTexture(e->sdl.renderer, str);
	if (texture == NULL)
		ft_error(str);
	return (texture);
}
...
int		ft_init_textures(t_env *e)
{
	...
	e->sdl.textures[BACKGROUND] = ft_load_texture(e,PATH"/resources/swag.png");
	return (1);
}
...
int		ft_init_sdl(t_env *e)
{
	...
	if (SDL_Init(SDL_INIT_EVERYTHING) != 0)
		return (0);
	e->sdl.window = SDL_CreateWindow("lem-in visu", e->sdl.screen.x, e->sdl.screen.y,
	...
	if (SDL_SetRenderDrawColor(e->sdl.renderer, 0, 0, 0, 255) < 0)
		return (0);
	e->sdl.surface = SDL_CreateRGBSurface(0, WIN_WIDTH, WIN_HEIGHT, 32, 0, 0, 0, 0);
